...
public class ActivityInfoServiceImpl extends ServiceImpl<ActivityInfoMapper, ActivityInfo> implements ActivityInfoService {
    ...
    public OrderConfirmVo findCartActivityAndCoupon(List<CartInfo> cartInfoList,
        ...
        /*
        整体逻辑：
        获取购物车中每个购物项参与的活动，根据活动规则进行分组，计算商品参与活动之后的金额是多少
        获取购物车中可以使用的优惠券列表 计算商品使用优惠券之后的金额（一次只能使用一张优惠券）
        计算没有参与活动，没有使用优惠券的原始金额  计算参与活动，使用优惠券之后的金额
        封装需要的数据返回OrderConfirmVo
         */
    ...
    public List<SkuInfo> findSkuInfoByKeyword(String keyword) {
        ...
        List<Long> existSkuIdList = baseMapper.selectSkuIdListExist(skuIdList);  //得到的是存在的集合

        //// (2) 判断逻辑处理:排除已经参加活动商品
        List<SkuInfo> findSkuList = new ArrayList<>();
        //遍历全部sku列表
        for (SkuInfo skuInfo:skuInfoList) {
            if(!existSkuIdList.contains(skuInfo.getId())) {  //不包含的放入集合
